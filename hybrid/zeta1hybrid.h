...
double calculate_my_sum(double *local_values, int count, int n_threads_omp);
...
double zeta1_hybrid(int n_threads_omp, int iterations, int size, int rank) {
    ...
    double my_sum = calculate_my_sum(local_values, local_count, n_threads_omp);
    double sum = 0;
...
double calculate_my_sum(double *local_values, int count, int n_threads_omp) {
	double sum = 0;
    #pragma omp parallel for num_threads(n_threads_omp) reduction(+:sum)
