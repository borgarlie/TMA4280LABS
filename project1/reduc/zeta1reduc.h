...
// my own implementation of all reduce (recursive doubling)
double reduc(int size, int rank, double my_sum) {
    // initialise sum with the process own sum
    double sum = my_sum;
    int d = 1;
    while (d < size) {
        // exchange local accumulated sum with rank XOR d
        int exchange = rank ^ d;
        double recv = 0;
        MPI_Sendrecv(&sum, 1, MPI_DOUBLE, exchange, 0, 
                &recv, 1, MPI_DOUBLE, exchange, MPI_ANY_TAG, 
                MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        // add the received accumulated sum to local accumulated sum
        sum += recv;
        // left shift d by 1
        d = d << 1;
    }
